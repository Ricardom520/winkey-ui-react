...
export default class Empty extends React.Component<EmptyProps> {
  ...
  render() {
    ...
    return (
      <div className={'wk-empty' + (className ? ` ${className}` : '')}>
        <div className='wk-empty-image' style={imageStyle}>
          {image && typeof image === 'string' ? (
            <img src={image} alt='empty' className='wk-empty-img-default' />
          ) : (
            image
          )}
          {!image && <img src={emptyImage} alt='empty' className='wk-empty-img-default' />}
        </div>
        {description !== false && <div className='wk-empty-description'>{description}</div>}
