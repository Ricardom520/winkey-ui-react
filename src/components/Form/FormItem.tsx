...
export default class FormItem extends React.Component<FormItemProps, FormItemState> {
    ...
    const childType = {
      button: (
        <Button
          {...kid.props}
          key={key}
          onClick={(e) => this.formButtonClick(e, kid.props)}
          size={size}
        />
      ),
      select: (
        <Select
          ...
          size={size}
          key={key}
          onChange={(e) => this.formSelectChange(e, kid.props)}
        />
      ),
      input: (
        <Input
          ...
          size={size}
          key={key}
          onChange={(e) => this.formInputChange(e, kid.props)}
        />
      ),
      password: (
        <Input
      ...
      checkbox: (
        <Checkbox
          ...
          size={size}
          key={key}
          onChange={(e) => this.formCheckBoxChange(e, kid.props)}
        />
      ),
      radioGroup: (
        <Radio.Group
          ...
          size={size}
          key={key}
          onChange={(e) => this.formRadioGroupChange(e, kid.props)}
        />
      ),
      datepicker: (
        <DatePicker
          ...
          size={size}
          key={key}
          onChange={(e) => this.formDatePickerChange(e, kid.props)}
        />
      ),
      switch: (
        <Switch
          ...
          size={size}
          key={key}
          onChange={(boolean) => this.formSwitchChange(boolean, kid.props)}
        />
      ),
      timepicker: (
        <TimePicker
          ...
          size={size}
          key={key}
          onChange={(e) => this.formTimePickerChange(e, kid.props)}
        />
      ),
      radio: (
        <Radio
          ...
          size={size}
          key={key}
          onChange={(e) => this.formRadioChange(e, kid.props)}
        />
      ),
      checkGroup: (
        <Checkbox.Group
          ...
          size={size}
          key={key}
          onChange={(e) => this.formCheckboxGroupChange(e, kid.props)}
        />
      )
    }
  ...
  render() {
    ...
    return (
      ...
      >
        {label && (
          <Col className={'wk-form-item-label' + colClass[labelAlign]} {...labelCol}>
            <label
              htmlFor={`${formName}_${label}`}
              title={label}
              className={rules && rules[0].required && 'wk-form-item-required'}
            >
              {label}
            </label>
          </Col>
        ...
        <Col {...wrapperCol} className='wk-form-item-control'>
          <div className='wk-form-item-control-input'>
            <div className='wk-form-item-control-input-content'>
              {!children.length && this.initChildren(children)}
              {children.length > 0 &&
                children.map((i, n) => {
                  return this.initChildren(i, n)
                })}
            </div>
          </div>
          {isError && (
            <div className={'wk-form-item-explain' + ' wk-form-item-explain-error'}>{isError}</div>
          )}
        </Col>
      </Row>
    )
