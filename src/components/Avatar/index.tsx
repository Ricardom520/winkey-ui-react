...
export default class Avatar extends React.Component<AvatarProps, AvatarState> {
  ...
  render() {
    ...
    return (
      <span
        className={'wk-avatar' + (sizeClass ? sizeClass : '') + avatarClass[shape]}
        style={{ ...avatarStyle, ...style }}
      >
        {icon && <i className={`iconfont ${icon}`} style={iconStyle}></i>}
        {children && (
          <span
            className='wk-avatar-string'
            style={{
              transform: `scale(${typeof size === 'number' ? 27 / size : 1}) translateX(-50%)`,
              lineHeight: typeof size === 'number' ? `${size}px` : null
            }}
          >
            {children}
          </span>
