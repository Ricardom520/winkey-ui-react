...
export default class AvatarPreview extends React.Component<AvatarPreviewProps, AvatarPreviewState> {
  ...
  imagePreviewMouseDown = (e) => {
    ...
    target.addEventListener('mousemove', (newE) => {
      newE.preventDefault()
      if (dragable) {
        ;(nowX = newE.clientX), (nowY = newE.clientY), (disX = nowX - initX), (disY = nowY - initY)
            ...
            <div className='wk-image-preview-body'>
              {!isMobile && (
                <ul className='wk-image-preview-operations'>
                  ...
                  <li
                    className='wk-image-preview-operations-operation'
                    onClick={() =>
                      this.setState({
                        scale: scale >= 10 ? 10 : scale + 0.5
                      })
                    }
                  >
                    <i
                      className={`iconfont wk-icon-enlarge wk-image-preview-icon ${
                        scale === 10 ? 'wk-image-preview-extreme' : ''
                      }`}
                    />
                  </li>
                  <li
                  ...
                  >
                    <i
                      className={`iconfont wk-icon-narrow wk-image-preview-icon ${
                        scale === 1 ? 'wk-image-preview-extreme' : ''
                      }`}
                    />
                  </li>
                  <li
