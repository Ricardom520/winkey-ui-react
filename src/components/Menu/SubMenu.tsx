...
export default class SubMenu extends React.Component<SubMenuProps, SubMenuState> {
    ...
    return (
      ...
      >
        <div ref={this.wkSubMenu} className='wk-menu-submenu-title'>
          {icon && <span className='wk-menu-item-icon anticon anticon-mail'>{icon}</span>}
          <span className='wk-menu-title-content'>{title}</span>
          {mode === 'inline' && <i className='wk-menu-submenu-arrow' />}
        </div>
        {mode === 'inline' && (
  ...
  render() {
    ...
    return (
      <>
        {this.initSubMenu()}
        <SubMenuBox top={offsetTop} left={offsetLeft} width={width} open={open}>
          {children}
        </SubMenuBox>
      </>
    )
