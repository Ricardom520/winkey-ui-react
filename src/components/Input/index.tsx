...
export default class Input extends Component<InputProps, IndexState> {
  ...
  render() {
    const { disabled, isMobile, size, addonBefore, suffix, prefix, addonAfter, allowClear } =
      this.props
    const { focus, indexValue } = this.state

    return (
      <>
        {!addonBefore && !suffix && !prefix && !addonAfter && !allowClear && this.initInput()}
        ...
        {!addonBefore && !addonAfter && (prefix || suffix || allowClear) && (
          ...
          >
            <span className='wk-input-prefix'>
              {typeof prefix === 'string' && prefix.includes('wk') ? (
