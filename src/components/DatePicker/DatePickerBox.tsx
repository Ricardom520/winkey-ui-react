...
export default class DatePickerBox extends React.Component<DatePickerBoxProps, DatePickerBoxState> {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      activeYearRange: [
        parseInt(`${year}`.substring(0, 3) + '0'),
        parseInt(`${year}`.substring(0, 3) + '9')
      ],
      activeDecadeRange: [
        parseInt(`${year}`.substring(0, 2) + '00'),
        parseInt(`${year}`.substring(0, 2) + '99')
      ],
      activeIndex: 0,
  ...
  componentDidMount() {
    ...
    if (!value && !defaultValue) {
      ...
      this.setState({
        ...
        activeYearRange: [
          parseInt(`${date[0]}`.substring(0, 3) + '0'),
          parseInt(`${date[0]}`.substring(0, 3) + '9')
        ],
        activeDecadeRange: [
          parseInt(`${date[0]}`.substring(0, 2) + '00'),
          parseInt(`${date[0]}`.substring(0, 2) + '99')
        ]
      })
    }
  ...
  handleSuperNext = () => {
    ...
    if (activePicker === 'month') {
      ...
      this.setState({
        activeYear: newYear,
        activeYearRange: [
          parseInt(`${newYear}`.substring(0, 3) + '0'),
          parseInt(`${newYear}`.substring(0, 3) + '9')
        ],
        activeDecadeRange: [
          parseInt(`${newYear}`.substring(0, 2) + '00'),
          parseInt(`${newYear}`.substring(0, 2) + '99')
        ]
      })
      ...
      if (activePicker !== 'year') {
        if (self) {
          ...
          this.setState(
            {
              activeDecadeRange: [
                parseInt(value.split('-')[0].substring(0, 2) + '00'),
                parseInt(value.split('-')[0].substring(0, 2) + '99')
              ]
            },
            () => {
              this.initDecade()
  ...
  retContent() {
    ...
    return (
      <div>
        <div
          className={'wk-datepicker-dropdown' + (!open ? ' wk-datepicker-dropdown-hidden' : '')}
          style={{
            top,
            left,
        ...
        >
          <div className='wk-datepicker-panel-container'>
            <div className='wk-datepicker-panel'>
              <div className={`wk-datepicker-${activePicker}-panel wk-datepicker-type-panel`}>
                <div className='wk-datepicker-header'>
                  ...
                  <div className='wk-datepicker-header-view'>
                    {activePicker === 'year' && (
                      <button className='wk-datepicker-decade-btn' onClick={this.initDecade}>
                        {activeYearRange[0]}-{activeYearRange[1]}
                      </button>
                    )}
                    {(activePicker === 'month' || activePicker === 'date') && (
                      <button className='wk-datepicker-year-btn' onClick={this.initYear}>
                        {activeYear}年
                      </button>
                    )}
                    {activePicker === 'date' && (
                      <button className='wk-datepicker-month-btn' onClick={this.initMonth}>
                        {activeMonth}月
                      </button>
                    )}
                              ...
                              return (
                                <td
                                  ...
                                  className={
                                    ...
                                    (n * i.length + d === activeIndex
                                      ? ' wk-datepicker-cell-selected'
                                      : '') +
                                    (j.disabled ? ' wk-datepicker-cell-disabled' : '')
                                  }
                                  onClick={() =>
                                    this.handleClick(j.value, j.label, n * i.length + d, j.self)
                                  }
                                >
                                  <div
                                    className={
                                      'wk-datepicker-cell-inner' +
                                      (`${activeYear}-${activeMonth}-${j.self && j.value}` === today
                                        ? ' wk-datepicker-cell-today'
                                        : '')
                                    }
                                  >
                                    {j.label}
